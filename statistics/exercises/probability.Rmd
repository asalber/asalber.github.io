...
1. $P(\mbox{1 heads})=`r dbinom(1,3,0.5)`$. <br/>
2. $P(\mbox{2 tails})=`r dbinom(2,3,0.5)`$. <br/>
3. $P(\mbox{2 or more heads})=`r pbinom(1,3,0.5,lower.tail=FALSE)`$. <br/>
4. $P(\mbox{some tails})=`r pbinom(0,3,0.5,lower.tail=FALSE)`$.
</div>
...
sg2 = 0.9
sg3 = 1
s = g1*sg1+g2*sg2+g3*sg3
n = 10000
```

A severe pain without effusion in a particular zone of the knee joint is a symptom of sprained lateral collateral ligament (SLCL). If the sprains in that ligament are classified into grade 1, when there is only distension (`r g1*100`% of cases), grade 2 when there is a partial tearing (`r g2*100`% of cases) and grade 3 when there is a complete tearing (`r g3*100`% of cases). Taking into account that the symptom appears in `r sg1*100`% of cases of grade 1 sprains, `r sg2*100`% of cases of grade 2 and `r sg3*100`% of cases of grade 3, answer the following questions:
...
3.  From a total of `r n` people with severe pain without effusion, how many are expected to have a grade 1 sprain? How many are expected to have a grade 2 sprain? And a grade 3 sprain?
...
1. $P(S)=`r s`$. <br/>
2. $P(G1\vert S)=`r g1*sg1/s`$, $P(G2\vert S)=`r g2*sg2/s`$ and $P(G3\vert S)=`r g3*sg3/s`$, so the diagnosis is a SLCL of grade 1. <br/>
3. $`r g1*sg1/s*n`$ will have a grade 1 sprain, $`r g2*sg2/s*n`$ will have a grade 2 sprain and $`r g3*sg3/s*n`$ will have a grade 3 sprain.
</div>
